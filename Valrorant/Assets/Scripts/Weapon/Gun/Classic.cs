...
public class Classic : Gun
{
    public override void Initialize(ClassicData data, RecoilRangeData mainRangeData, RecoilRangeData subRangeData)
    {
        ...
        _actionStrategies[EventType.Sub] = new ScatterProjectileAttackWithWeight(data.weaponName, data.range, _targetLayer, data.subFireCnt,
            ...
            OnPlayWeaponAnimation, ReturnMuzzlePos, ReturnLeftAmmoCount, DecreaseAmmoCount, SpawnMuzzleFlashEffect, SpawnEmptyCartridge, OnGenerateNoiseRequest, PlaySFX);
        ...
        _reloadStrategy = new MagazineReload(_weaponName, data.reloadFinishDuration, data.reloadExitDuration, data.maxAmmoCountInMagazine, OnPlayWeaponAnimation, OnReloadRequested, PlaySFX);
    }
}
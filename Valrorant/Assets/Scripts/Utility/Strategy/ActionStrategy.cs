...
public class NoResult : ActionStrategy
{
    ...
    public override bool CanAct() { return false; }
...
public class ZoomStrategy : ActionStrategy
{
    ...
    public override bool CanAct() { return true; }
...
abstract public class PenetrateAttack : ApplyAttack, IDisplacement
{
    ...
    {
        // _muzzle 사용
        OnNoiseGenerateRequested?.Invoke(_muzzle.position);
    ...
    public override bool CanAct() { return _leftBulletCount > 0; }
...
public class SingleAndExplosionScatterAttackCombination : ActionStrategy // Attack이 아니라 다른 방식으로 명명해야할 듯
{
        ...
        , Action<Vector3> OnNoiseGenerateRequested)
    {
        ...
        singleProjectileAttack = new SingleProjectileAttack(camTransform, range, targetLayer, ownerAnimator, weaponAnimator, muzzleFlashSpawner, canSpawnMuzzleFlash, emptyCartridgeSpawner, isMainAction,
            weaponName, muzzle, singlePenetratePower, trajectoryLineEffect, singleBulletSpreadPowerDecreaseRatio, singleDamageDictionary, OnNoiseGenerateRequested);
        ...
        scatterProjectileGunAttack = new ExplosionScatterProjectileAttack(camTransform, range, targetLayer, bulletCountsInOneShoot, ownerAnimator, weaponAnimator, muzzleFlashSpawner, canSpawnMuzzleFlash,
            ...
            frontPosition, explosionEffectName, scatterDamageDictionary, OnNoiseGenerateRequested);
    }
    ...
    public override bool CanAct() { return singleProjectileAttack.CanAct() && scatterProjectileGunAttack.CanAct(); }
...
public class SingleProjectileAttack : PenetrateAttack
{
    ...
    {
        float x = UnityEngine.Random.Range(-weight, weight);
        float y = UnityEngine.Random.Range(-weight, weight);
...
public class ScatterProjectileAttack : PenetrateAttack // 산탄은 가중치가 적용되지 않음
{
    ...
    List<Vector3> ReturnOffsetDistance(float weight, int fireCount)
    {
        ...
        {
            float x = UnityEngine.Random.Range(-_spreadOffset - weight, _spreadOffset + weight);
            float y = UnityEngine.Random.Range(-_spreadOffset - weight, _spreadOffset + weight);
